...
namespace Mybot.ChromeDevTools.Protocol.Chrome.Tracing
{
	...
	public class StartCommand: ICommand<StartCommandResponse>
	{
		...
		/// <summary>
		/// Gets or sets Trace data format to use. This only applies when using `ReturnAsStream` transfer mode (defaults to `json`).
		/// </summary>
		[JsonProperty(NullValueHandling = NullValueHandling.Ignore)]
		public string StreamFormat { get; set; }
		/// <summary>
		/// Gets or sets Compression format to use. This only applies when using `ReturnAsStream` transfer mode (defaults to `none`)
		/// </summary>
		[JsonProperty(NullValueHandling = NullValueHandling.Ignore)]
		...
		/// <summary>
		/// Gets or sets Base64-encoded serialized perfetto.protos.TraceConfig protobuf message When specified, the parameters `categories`, `options`, `traceConfig` are ignored. (Encoded as a base64 string when passed over JSON)
		/// </summary>
		[JsonProperty(NullValueHandling = NullValueHandling.Ignore)]
