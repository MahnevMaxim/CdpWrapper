...
namespace Mybot.ChromeDevTools.Protocol.Chrome.Browser
{
	...
	{
		/// <summary>
		/// Gets or sets Whether to allow all or deny all download requests, or use default Chrome behavior if available (otherwise deny). |allowAndName| allows download and names files according to their dowmload guids.
		/// </summary>
		...
		/// <summary>
		/// Gets or sets The default path to save downloaded files to. This is required if behavior is set to 'allow' or 'allowAndName'.
		/// </summary>
		[JsonProperty(NullValueHandling = NullValueHandling.Ignore)]
